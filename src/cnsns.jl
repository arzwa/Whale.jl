...
majority vote.
"""
function consensus_tree_reconciliation!(T::ConRecTree, rectrees::Array{RecTree})
...
hashnode(node::Int64, tree::AbstractRecTree) = hash(Set([tree.leaves[n] for n in
...
# Majority consensus tree construction =========================================
# I need to implement this in order to make the library not too cumbersome
# NOTE: Would make more sense to implement in the PhyloTrees library?

# This hashes a RecTree ========================================================
# a recnode is uniquely define by it's label, species, and set of leaves
# count all recnodes, sort them by descending #occurrences, then #tips,
# then tip order
function count_recnodes(trees)
    d = hashrectree(trees[1])
    for i in 2:length(trees)
        merge!(+, d, hashrectree(trees[i]))
    end
    return d
end

function hashrectree(rectree)
    hashes = Dict{UInt64,Int64}()
    function walk(n)
        if isleaf(rectree.tree, n)
            l = Set([rectree.leaves[n]])
        else
            l = union([walk(c) for c in childnodes(rectree.tree, n)]...)
        end
        hashes[hash((rectree.labels[n], rectree.σ[n], l))] = 1
        return l
    end
    walk(findroots(rectree.tree)[1])
    return hashes
end
