...
begin
    ...
    w.da = true
    for i=1:100
        @time mcmc!(w, 100, show_every=1, backtrack=false)
        p = plot(stephist(w.df[!,:λ1],alpha=0.2,fill=true,color=:black,normalize=true),
                 stephist(w.df[!,:μ1],alpha=0.2,fill=true,color=:black,normalize=true),
                 stephist(w.df[!,:q1],alpha=0.2,fill=true,color=:black,normalize=true),
                 stephist(w.df[!,:q2],alpha=0.2,fill=true,color=:black,normalize=true),
                 stephist(w.df[!,:η], alpha=0.2,fill=true,color=:black,normalize=true),
                 plot(log.(w.df[!,:λ1]), color=:black, linewidth=1.5),
                 plot(log.(w.df[!,:μ1]), color=:black, linewidth=1.5),
                 legend=false, grid=false)
        plot!(p[1], color=:black, w.prior.λ, linewidth=2)
        plot!(p[2], color=:black, w.prior.λ, linewidth=2)
        plot!(p[3], color=:black, w.prior.q, linewidth=2)
        plot!(p[4], color=:black, w.prior.q, linewidth=2)
        plot!(p[5], color=:black, w.prior.η, linewidth=2)
        display(p)
    end
end
