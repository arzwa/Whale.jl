...
function read_ale_from_list(list::String, S::SpeciesTree)
    ...
    open(list, "r") do f
        lines = readlines(f)
        @showprogress 1 "Reading ALE files " for line in lines
            try
...
Get CCDs either a single file, list of files or directory with files
generated by ALEobserve (ALE package).
"""
...
# helper methods
isambiguousleaf(γ::Int64, ccd::CCD, S::SpeciesTree) = haskey(
    S.ambiguous, ccd.m3[γ])
isambiguousleafof(γ::Int64, ccd::CCD, e::Int64, S::SpeciesTree) =
    (isleaf(S.tree, e) && haskey(S.ambiguous, ccd.m3[γ]))
